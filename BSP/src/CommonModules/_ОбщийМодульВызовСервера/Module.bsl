...
Функция УстановитьСтатусДокументов(Знач МассивДокументов, Знач НовыйСтатус, Знач ДополнительныеПараметры=Неопределено) Экспорт
	...
	Для Каждого СтрМассивКУдалению Из МассивКУдалению Цикл
		МассивДокументов.Удалить(МассивДокументов.Найти(СтрМассивКУдалению));	
	КонецЦикла;
...
//
			Если НЕ ДополнительныеПараметры = Неопределено И Выборка.СтатусСовпадает И НЕ ДополнительныеПараметры.Свойство("СтатусСогласования") Тогда
...
Функция МожноОтправитьЗаявку(Заявка)  Экспорт
	...
	Если  Пользователь = Заявка.Инициатор ИЛИ РегистрыСведений._ГруппыДоступа.ДоступРазрешен(Заявка.Инициатор) Тогда
		Результат = истина; 
	Иначе
...
Функция ПолучитьЛимитыПоСтатьиВЗаявкеДляСписка(Заявка) Экспорт
	
	Результат = Новый Структура("БюджетныйЛимит,Исчерпано,Остаток,Инфо",0,0,0,"");


	Запрос = Новый Запрос;
	Запрос.Текст =
		"ВЫБРАТЬ
		|	_ДвиженияДенежныхСредствОстаткиИОбороты.СуммаПлатежаКонечныйОстаток КАК Остаток,
		|	_ДвиженияДенежныхСредствОстаткиИОбороты.СуммаПлатежаПриход КАК БюджетныйЛимит,
		|	_ДвиженияДенежныхСредствОстаткиИОбороты.СуммаПлатежаРасход КАК Исчерпано
		|ИЗ
		|	РегистрНакопления._ДвиженияДенежныхСредств.ОстаткиИОбороты(,,,, КалендарныйПлатеж = &КалендарныйПлатеж
		|	И СтатьяДДС В (&СтатьяДДС)
		|	И ЦФО В (&ЦФО)
		|	И Инициатор = &Инициатор) КАК _ДвиженияДенежныхСредствОстаткиИОбороты";
...
//BSP-11 Механизм уведомлений
Функция ОтправитьПисьмаСразу() Экспорт

	Возврат Константы._ОтправитьПисьмаСразу.Получить();	
...
Процедура ИзменениеСпискаСогласователейПриЗаписи(Источник, Отказ) Экспорт
	
	Источник.СогласователиДляСписка  = "";
	Если Источник.РазбивкаПоСтатьям.количество() > 1 тогда
		Согласователи = _ОбщийМодульВызовСервера.ПолучитьСтруктуруПисемПриНесколькихСтатей(Источник.ссылка);
		Для каждого согл из Согласователи цикл
			 Источник.СогласователиДляСписка = Источник.СогласователиДляСписка + Согл.наименование + "; " ;
		КонецЦикла;
	Иначе
		Если Источник.ПервыйСогласован тогда
			Источник.СогласователиДляСписка  = Источник.ВторойСогласователь.Наименование;
		Иначе
			Источник.СогласователиДляСписка  = Источник.Согласователь.Наименование;
		КонецЕсли;
	КонецЕсли;
КонецПроцедуры

Процедура ИзменениеФорматаПисьмаПередЗаписью(Источник, Отказ, РежимЗаписи, РежимПроведения) Экспорт
	Источник.текстHTML = Источник.Текст;
