...
Функция СформироватьСообщение(ПараметрыОтправки) Экспорт
	...
	Если МенеджерОбъекта <> Неопределено Тогда
		Если ПараметрыОтправки.дополнительныеПараметры.свойство("Контакт") тогда
	...
	//++BSP  соответствие
	//ПараметрыОтправки.ДополнительныеПараметры.ПроизвольныеПараметры.Вставить("СрочнаяЗаявка", "Срочная заявка");
	Параметр_Произвольные = ПараметрыТекстаСообщения.Получить("Произвольные");
	Если Параметр_Произвольные <> Неопределено Тогда
		Если ТипЗнч(ПараметрыОтправки.Предмет) = Тип("ДокументСсылка._ЗаявкаНаОплату") Тогда
			ТекстСрочнаяЗаявка = ?(НачалоДня(ПараметрыОтправки.Предмет.ДатаОплаты) = НачалоДня(ТекущаяДата()), "Срочная заявка", "");
			Параметр_Произвольные.Вставить("СрочнаяЗаявка", ТекстСрочнаяЗаявка);
		КонецЕсли;
	КонецЕсли;
	//--BSP
