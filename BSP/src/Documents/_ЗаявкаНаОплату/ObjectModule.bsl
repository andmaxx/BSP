...
Функция УстановитьСтатус(НовыйСтатус, ДополнительныеПараметры) Экспорт
	...
	Если НЕ ДополнительныеПараметры = Неопределено 
		И ДополнительныеПараметры.Свойство("СтатусСогласования") Тогда
		СтатусСогласования = Перечисления._СтатусыЗаявок[ДополнительныеПараметры.СтатусСогласования];
	КонецЕсли;
	//
...
Процедура ИзменитьОригиналПолучен() Экспорт
...
Процедура ПередЗаписью(Отказ, РежимЗаписи, РежимПроведения)
	...
	Если ПометкаУдаления И НЕ Ссылка.ПометкаУдаления Тогда
		Если 
			...
			И НЕ ПараметрыСеанса.ТекущийПользователь = Ссылка.Инициатор 
		Тогда
			Отказ = Истина;
			ОбщегоНазначенияКлиентСервер.СообщитьПользователю("Удаление заявок доступно только инициатору", ,,Отказ);		
		КонецЕсли;
		Если 
			Статус = Перечисления._СтатусыЗаявок.Оплачено
		Тогда
			Отказ = Истина;
	...
	Для каждого стр из РазбивкаПоСтатьям Цикл
		  СтатьиДляСписка = СтатьиДляСписка + стр.СтатьяДДС.Код  + " " + Строка(стр.СтатьяДДС) + "; " ;
...
Процедура ПриЗаписи(Отказ)
	Если ЗначениеЗаполнено(Статус) Тогда
		...
		//Если Тз.Количество() = 0 Тогда
			...
			Запись.СтатусСогласования = СтатусСогласования;
