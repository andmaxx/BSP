...
//--
	...
	//BSP-8	Механизм редактирования заявок
	Элементы.УстановитьСтатусПозвонить.Доступность     = _ОбщийМодульВызовСервера.ЕстьДоступУстановитьСтатус(Перечисления._СтатусыЗаявок.Согласован);
...
&НаКлиенте
Процедура УстановитьСтатусПозвонить(Команда)
	ВыделенныеСтроки  = Новый Массив;
	ВыделенныеСтроки.Добавить(Объект.Ссылка);
	
	ТекстВопроса = НСтр("ru='У заявки будет установлен статус ""Позвонить"". Продолжить?'");
	Ответ = Неопределено;
	
	ПоказатьВопрос(Новый ОписаниеОповещения("УстановитьСтатусПозвонитьЗавершение", ЭтотОбъект, Новый Структура("ВыделенныеСтроки", ВыделенныеСтроки)), ТекстВопроса,РежимДиалогаВопрос.ДаНет);
...
&НаКлиенте
Процедура УстановитьСтатусПозвонитьЗавершение(РезультатВопроса, ДополнительныеПараметры) Экспорт
	...
	ДопПараметры = Новый Структура("СтатусСогласования", "Позвонить");	
    ОбработанныеДокументы = _ОбщийМодульВызовСервера.УстановитьСтатусДокументов(ВыделенныеСтроки, "На согласовании", ДопПараметры);
    
    _ОбщийМодульКлиент.ОповеститьПользователяОбУстановкеСтатуса(Неопределено, ОбработанныеДокументы, ВыделенныеСтроки.Количество(), НСтр("ru='Позвонить'"));
   	ОбновитьФормуНаСервере();
КонецПроцедуры
...
Процедура УстановитьСтатусДоработатьЗавершение(РезультатВопроса, ДополнительныеПараметры) Экспорт
	...
	ДопПараметры = Новый Структура("СтатусСогласования", "Доработать");	
    ОбработанныеДокументы = _ОбщийМодульВызовСервера.УстановитьСтатусДокументов(ВыделенныеСтроки, "На согласовании", ДопПараметры);
