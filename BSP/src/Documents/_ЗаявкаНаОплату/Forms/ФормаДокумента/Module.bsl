...
Процедура ОбновитьСсылкуЭтапыОплатыНаСервере()
	Элементы.ЭтапыОплатыРазбивка.Заголовок = 
	?(Объект.ЭтапыОплаты, 
	"Есть этапы оплаты" + "("+ РазбивкаНаЭтапыОплаты.Количество()+")", 
	"Создать этапы оплаты");
	...
	Попытка
		Для Каждого СтрЭтап ИЗ ТЗЭтапы Цикл
			Если СтрЭтап.НомерСтроки = 1 Тогда
				...
				Объект.ЭтапыОплаты = Истина;
				Объект.СуммаВсехЭтапов = Объект.СуммаПлатежа;
				Объект.СуммаПлатежа = СтрЭтап.Сумма;
			...
			Иначе
				...
				ЗаявкаЭтапаОбъект.Дата = СтрЭтап.ДатаОплаты;
...
Функция ПолучитьТаблицуЭтаповОплат()
	...
	Запрос.УстановитьПараметр("ОсновнаяЗаявка", Объект.Ссылка);
