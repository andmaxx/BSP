...
Процедура ПриСозданииНаСервере(Отказ, СтандартнаяОбработка)
	...
	//--
	//BSP-9
	Элементы.ПояснениеНДС.Видимость = ПрименяетсяНДС(Объект.Плательщик) И Объект.ПрименяетсяНДС <> ПрименяетсяНДС(Объект.Плательщик);
	//--
	//++BSP fix 5
	//добавлена настройка в условное оформление формы
	УстановитьРеквизитЭтоЮрлицо();
	//--BSP fix 5
КонецПроцедуры
...
//++BSP fix 5
&НаСервере
Функция ЗначениеРеквизитаОбъектаНаСервере(РеквизитСсылка, ИмяРеквизита)
	Возврат ОбщегоНазначения.ЗначениеРеквизитаОбъекта(РеквизитСсылка, ИмяРеквизита);
КонецФункции

&НаСервере
Процедура УстановитьРеквизитЭтоЮрлицо()
	Для Каждого СтрРазбивкаПоСтатьям Из Объект.РазбивкаПоСтатьям Цикл
		Если ЗначениеЗаполнено(СтрРазбивкаПоСтатьям.ЦФО) Тогда
			СтрРазбивкаПоСтатьям.ЭтоЮрлицо = ЗначениеРеквизитаОбъектаНаСервере(СтрРазбивкаПоСтатьям.ЦФО, "ЭтоЮрЛицо");
		КонецЕсли;	
	КонецЦикла;
КонецПроцедуры

&НаКлиенте
Процедура РазбивкаПоСтатьямЦФОПриИзменении(Элемент)
	Если ЗначениеЗаполнено(Элементы.РазбивкаПоСтатьям.ТекущиеДанные.ЦФО) Тогда
		Элементы.РазбивкаПоСтатьям.ТекущиеДанные.ЭтоЮрлицо = ЗначениеРеквизитаОбъектаНаСервере(Элементы.РазбивкаПоСтатьям.ТекущиеДанные.ЦФО, "ЭтоЮрЛицо");	
	КонецЕсли;
КонецПроцедуры
//--BSP fix 5
